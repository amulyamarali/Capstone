...
def compute_metrics(data):
    ...
    precision = precision_score(
        edge_label.cpu().numpy(), predictions.cpu().numpy())
...
node_feature_list = np.array(
    [entity_embeddings[entity_to_id[entity]] for entity in entities])
...
with torch.no_grad():
    ...
    original_node = get_key_from_value(
        sparse_node_indices_map, most_likely_link_node)

    print("node index before mapping: ", most_likely_link_node)
    print("original node index: ", original_node)

    print(
        f"The new node is most likely to link with node {original_node} with the score of {new_edge_predictions[most_likely_link_index]}.")
